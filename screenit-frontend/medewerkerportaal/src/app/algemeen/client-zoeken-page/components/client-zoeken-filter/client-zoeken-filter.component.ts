...
import { briefkenmerkValidator, huisnummerValidator, trimmedValidator } from '@shared/validators/common-validators'
...
import { mobieleTelefoonValidator } from '@shared/validators/telefoon-nummer/telefoon-nummer.validators'
...
export class ClientZoekenFilterComponent {
  ...
  zoekenForm = this.formBuilder.group({
    ...
    mobielnummer: this.formBuilder.control<string | null>(null, mobieleTelefoonValidator),
  ...
  zoekClienten() {
    ...
    if (this.zoekenForm.valid && this.heeftGeldigeInput()) {
      ...
      const formWaarden = Object.fromEntries(Object.entries(this.zoekenForm.value).map(([key, value]) => [key, typeof value === 'string' ? value.trim() : value]))
