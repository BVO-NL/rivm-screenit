...
@Slf4j
@Service
public class MammaDicomCStoreServiceProviderImpl implements ApplicationListener<ContextRefreshedEvent>
{
	@Autowired
	...
	private DicomServiceRegistry createServiceRegistry()
	{
		...
		serviceRegistry.addDicomService(new BasicCStoreSCP("*")
		{
				...
				throws IOException
			{
				...
				var file = new AtomicReference<>(File.createTempFile(iuid, ".part"));
				try
				{
					storeTo(as, as.createFileMetaInformation(iuid, cuid, tsuid), data, file.get());
					var accessionNumber = new AtomicReference<String>(null);
					var seriesNumber = new AtomicReference<String>(null);
					String instanceNumber;
					try (var din = new DicomInputStream(file.get()))
					{
						var attribs = din.readDataset(-1, o -> Integer.compareUnsigned(o.tag(), Tag.PatientOrientation) >= 0);
						accessionNumber.set(attribs.getString(Tag.AccessionNumber));
						seriesNumber.set(attribs.getString(Tag.SeriesNumber));
					...
					databaseRunner.runInSessionOnly(() ->
					{
						var downloadOnderzoek = downloadOnderzoekService.findDownloadOnderzoekInVerwerking(Long.parseLong(accessionNumber.get()));
						if (downloadOnderzoek.isPresent())
						{
							uitwisselPortaalService.kopieerDicomBestandNaarDownloadVerzoekMap(file.get(), seriesNumber.get(), downloadOnderzoek.get());
						}
						else
						{
