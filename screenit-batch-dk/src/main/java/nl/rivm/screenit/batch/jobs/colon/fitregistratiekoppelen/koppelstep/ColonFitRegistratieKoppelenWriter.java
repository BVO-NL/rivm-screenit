package nl.rivm.screenit.batch.jobs.colon.fitregistratiekoppelen.koppelstep;
...
public class ColonFitRegistratieKoppelenWriter implements ItemWriter<VERZONDENUITNODIGING>
{
	@Autowired
	...
	public void write(Chunk<? extends VERZONDENUITNODIGING> chunk) throws Exception
	{
		...
		for (VERZONDENUITNODIGING verzondenUitnodiging : chunk.getItems())
		{
			...
			try
			{
				...
				boolean barcodeGoldVerplicht = ColonOnderzoeksVariant.isOfType(onderzoeksVariant, ColonFitType.GOLD);
				boolean barcodeExtraVerplicht = ColonOnderzoeksVariant.isOfType(onderzoeksVariant, ColonFitType.STUDIE);
				...
				fitService.koppelTestIndienMogelijk(ifobtBarcodeGold, ColonFitType.GOLD, uitnodiging, datumVerstuurd, screeningRonde);
				fitService.koppelTestIndienMogelijk(ifobtBarcodeExtra, ColonFitType.STUDIE, uitnodiging, datumVerstuurd, screeningRonde);
