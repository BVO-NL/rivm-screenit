...
export const mobieleTelefoonValidator = (control: AbstractControl): ValidationErrors | null => {
  ...
  if (!control.dirty || isStringNullOfLeeg(waardeZonderSpatieOfStreepje)) {
    return null
  }

  const mobielTelefoonRegex = new RegExp(/^(?:(?:\+|00)31|0)6-?\d{8}$/)
  return mobielTelefoonRegex.test(waardeZonderSpatieOfStreepje) ? null : { mobielTelefoon: true }
}
...
export const normaalTelefoonnummerValidator = (control: AbstractControl): ValidationErrors | null => {
  ...
  if (isStringNullOfLeeg(waardeZonderSpatieOfStreepje)) {
    return null
  }
  ...
  return vastNummerPattern.test(waardeZonderSpatieOfStreepje) ||
