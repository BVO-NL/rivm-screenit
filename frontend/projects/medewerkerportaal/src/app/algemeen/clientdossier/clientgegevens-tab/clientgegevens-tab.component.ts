...
@Component({
  ...
  imports: [DsNavigationSidebarComponent, RouterOutlet],
...
export class ClientgegevensTabComponent {
  ...
  private readonly actieveBvos = toSignal(toObservable(this.client).pipe(switchMap((client) => (client ? this.clientService.getActieveBvos(client.id) : of([])))), {
    initialValue: [],
  })
  protected readonly clientDossierNavigationItems = computed(() => getClientDossierNavigationItems(this.actieveBvos()))
}
