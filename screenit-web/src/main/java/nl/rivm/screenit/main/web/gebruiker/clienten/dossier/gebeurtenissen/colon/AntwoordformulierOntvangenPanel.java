...
public class AntwoordformulierOntvangenPanel extends AbstractGebeurtenisDetailPanel
{
	...
	protected void addButton(String id, GebeurtenisPopupBasePanel parent)
	{
		...
		if (uitnodiging != null)
		{
			...
			{
				var testStatus = fitRegistratie.getStatus();
				if (ColonFitRegistratieStatus.ACTIEF.equals(testStatus) || ColonFitRegistratieStatus.VERWIJDERD.equals(testStatus))
				{
	...
	private class InfoFragment extends Fragment
	{
		...
		private String getFitStatus(ColonUitnodiging uitnodiging)
		{
			var registratie = ColonFitRegistratieUtil.getFitRegistratie(uitnodiging);
			return getString(EnumStringUtil.getPropertyString(registratie.getStatus()));
		}
	}
}
