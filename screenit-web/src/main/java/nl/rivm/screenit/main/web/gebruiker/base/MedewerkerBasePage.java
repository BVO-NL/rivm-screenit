...
public abstract class MedewerkerBasePage extends BasePage
{
	...
	public MedewerkerBasePage()
	{
		ScreenitSession.get().setInPlanningmodule(false);
		heeftImsKoppelingRecht = ScreenitSession.get().checkPermission(Recht.MEDEWERKER_SCREENING_MAMMA_IMS_KOPPELING, Actie.INZIEN);
		isMammaBeoordelaar = ScreenitSession.get().checkPermission(Recht.MEDEWERKER_SCREENING_MAMMA_BEOORDELING_WERKLIJST, Actie.AANPASSEN) &&
	...
	protected void onInitialize()
	{
		...
		add(new SessionCheckingPanel("zorgIdChecking")
		{
			...
			{
				MedewerkerBasePage.this.logout();
			}
		});
		...
		add(new ContextMenuPanel("contextMenu")
		{
			...
			{
				return MedewerkerBasePage.this.getActiveContextMenuClass();
			}

			@Override
			protected List<MedewerkerMenuItem> getAllowedContextMenuItems()
			{
				return MedewerkerBasePage.this.getAllowedContextMenuItems();
			}
		});
		...
		if (ingelogdeOrganisatieMedewerker != null)
		{
			if (autorisatieService.getActieVoorMedewerker(ingelogdeOrganisatieMedewerker, null, Recht.MEDEWERKER_BEHEER) != null)
			{
				naarMedewerkerGegevens = new AjaxLink<Object>("naarMedewerkerGegevens")
				{
					...
					{
						setResponsePage(new MedewerkerBasisgegevens(ModelUtil.cModel(getIngelogdeOrganisatieMedewerker().getMedewerker())));
					}
				};
			}
			naarMedewerkerGegevens.add(new Label("ingelogdeOrganisatieMedewerker", NaamUtil.getNaamMedewerker(ingelogdeOrganisatieMedewerker.getMedewerker())));
		}
		else
		{
			naarMedewerkerGegevens.add(new Label("ingelogdeOrganisatieMedewerker"));
		}
	...
	protected OrganisatieMedewerker getIngelogdeOrganisatieMedewerker()
	{
	...
	private void addBvoFilter()
	{
		...
		if (onderzoeken.size() < 2)
		{
			...
			if (onderzoeken.size() == 1 && !organisatieMedewerker.getBevolkingsonderzoeken().contains(onderzoeken.get(0)))
			{
	...
	private void addMenu()
	{
		ListView<MedewerkerMenuItem> hoofdmenu = new ListView<MedewerkerMenuItem>("hoofdMenu", getHoofdMenuItems())
		{
			...
			protected void populateItem(ListItem<MedewerkerMenuItem> item)
			{
				...
				if (CollectionUtils.isNotEmpty(medewerkerMenuItem.getSubMenuItems()))
				{
					...
					contextMenu = new SubMenuPanel("subMenu", medewerkerMenuItem.getSubMenuItems());
				}
				else
				{
	...
	private List<MedewerkerMenuItem> getHoofdMenuItems()
	{
	...
	protected abstract MedewerkerHoofdMenuItem getActieveMenuItem();
	...
	private void afmeldenConformAjaxDialog()
	{
		add(new ConfirmingIndicatingAjaxLink<Void>("afmelden", dialog, "afmelden")
		{
			...
			{
				return !getIngelogdeOrganisatieMedewerker().getMedewerker().getInlogMethode().equals(InlogMethode.UZIPAS);
			}
