...
function VoorspellingsgrafiekenDeelnamekans() {
	...
	$("#voorspellingsgrafiekDeelnamekansDiv").mousemove((event) => {
		deVoorspellingsgrafiekenDeelnamekans.mouseMove(event)
	})

	$("#voorspellingsgrafiekDeelnamekansDiv").mousedown((event) => {
		deVoorspellingsgrafiekenDeelnamekans.mouseDown(event)
	})

	$("#voorspellingsgrafiekDeelnamekansDiv").mouseup((event) => {
		deVoorspellingsgrafiekenDeelnamekans.mouseUp(event)
	})

	$("#js-voorspelling-grafiek").change((event) => {
	...
	leesDeelnamekansen(() => {
		...
		matrix2 = new VoorspellingsgrafiekMatrix(svgMatrix2, marginBottom, "De onderstaande groepen overlappen", ["Dubbele tijd", "Mindervalide", "Tehuis", "Suspect"])

		grafiekVervolgRonde.init(alleDeelnamekansen.kolommen.find((kolom) => {
			return kolom.naam === "Vervolgronde"
		}).cumulatieveDeelnamekansVerdeling)
		grafiekEersteRonde.init(alleDeelnamekansen.kolommen.find((kolom) => {
