...
public abstract class AbstractUitnodigingenVersturenTasklet<U extends InpakbareUitnodiging<?>> implements Tasklet
{
	...
	private static int NR_OF_RETRIES_ZIP_VERZENDEN;

	private static int TIME_BETWEEN_RETRIES_ZIP_VERZENDEN;
	...
	public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception
	{
		...
		{
			return handleAvailabilityException(contribution, e);
		}

		onsuccesvolleVerzendPogingenGehad = 0;
		NR_OF_RETRIES_ZIP_VERZENDEN = preferenceService.getInteger(PreferenceKey.RETRIES_VERZENDEN_INPAKCENTRUM.name());
		TIME_BETWEEN_RETRIES_ZIP_VERZENDEN = preferenceService.getInteger(PreferenceKey.TIME_BETWEEN_RETRIES_VERZENDEN_INPAKCENTRUM.name());
	...
	private void waitForAvailableEndpoint()
	{
		...
		if (i == NR_OF_TRIES)
		{
			...
			message.insert(0, "Na " + NR_OF_TRIES + " pogingen in het afgelopen uur is de Inpakcentrum endpoint niet bereikbaar geweest. ");
	...
	private void genereerUitnodigingen() throws Exception
	{
	...
	private void verstuurUitnodigingen(List<Long> uitnodigingIds) throws IOException
	{
		if (!uitnodigingIds.isEmpty())
		{
			LOG.info("Start versturen, uitnodigingIds size: {}, id van eerste uitnodiging: {}", uitnodigingIds.size(), uitnodigingIds.get(0));

			if (verstuurUitnodigingen())
			{
				LOG.info("Alles verstuurd met resultaat: {}", true);
	...
	private boolean verstuurUitnodigingen() throws IOException
	{
	...
	{
		if (versturenGeslaagd || onsuccesvolleVerzendPogingenGehad == NR_OF_RETRIES_ZIP_VERZENDEN)
		{
			onsuccesvolleVerzendPogingenGehad = 0;
			return false;
		}
		...
		try
		{
			LOG.info("Retry nodig, wacht {} seconden", TimeUnit.MILLISECONDS.toSeconds(TIME_BETWEEN_RETRIES_ZIP_VERZENDEN));
			Thread.sleep(TIME_BETWEEN_RETRIES_ZIP_VERZENDEN);
		}
		catch (InterruptedException e)
		{
			LOG.error("Fout in sleep", e);
	...
	private RepeatStatus handleAvailabilityException(StepContribution contribution, Exception e)
	{
